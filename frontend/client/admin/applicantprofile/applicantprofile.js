...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    if (applicantData.success && applicantData.applicant) {
      populateProfile(applicantData.applicant);
      loadProfilePicture(applicantData.applicant._id);
    } else {
      showNotification("Failed to load applicant data");
    }
  } catch (error) {
...
function populateProfile(applicantData) {
  if (!applicantData) {
  ...
  const personalInfo = applicantData.personalInfo || applicantData;
  ...
  if (idElement) {
    idElement.innerHTML = `<i class="fa-solid fa-id-card"></i> ${applicantData.applicantId || "Not assigned"}`;
  }

  const emailElement = document.getElementById("user-email");
  if (emailElement) {
    emailElement.innerHTML = `<i class="fa-solid fa-envelope"></i> ${
      personalInfo.emailAddress || applicantData.email || "N/A"
    }`;
  }
  ...
  for (const [htmlId, dataKey] of Object.entries(fieldMappings)) {
    ...
    if (element) {
      let value = personalInfo[dataKey] || applicantData[dataKey] || "N/A";
...
async function loadProfilePicture(userId) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Use default image if no profile picture found
      const profilePic = document.getElementById("profile-pic");
      if (profilePic) {
